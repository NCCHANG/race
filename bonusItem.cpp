...
struct Bonus {
    ...
    void moveForth(int xInitial, int height, int &racerLocation, int &racerX, int &racerY) {
        racerLocation += 3;
        calculateXY(xInitial, height, racerLocation, racerX, racerY);
    }

    void doubleSteps(int xInitial, int height, int &racerLocation, int &racerX, int &racerY) {
        racerLocation *= 2;
        calculateXY(xInitial, height, racerLocation, racerX, racerY);
    }

    void extraTurn(int step, int xInitial, int height, int &racerLocation, int &racerX, int &racerY) {
        racerLocation += step;
        calculateXY(xInitial, height, racerLocation, racerX, racerY);
    }

    void bonusItem_option(int &racerLocation, int &racerX, int &racerY, string racername, vector<int> &bonusFunc, int xInitial, int height, int step) {
        cout << "Enter input to use your bonus item (y = Use Bonus, x = Keep Bonus): ";
        char choice;
        cin >> choice;
        if (choice == 'y' || choice == 'Y') { // Logic to use the bonus immediately
            int bonusType = bonusFunc.back();
            if (bonusType == 1) {
                ...
                cout << racername << " BONUS ITEM OBTAINED! " << racername << ", your steps have been doubled" << endl << endl;
            } else if (bonusType == 3) {
...
int main()
{
    ...
    box_inquiry();
    lap_inquiry();
    bonus.bonusItem_inquiry();
    bonus.initializeBonusPositions();
    bonus.displayBonusInfo();
    l.printLayout();
    while (gameRunning) {
